...
- **铁律 2**：每次发布 **`versionCode` 必须 +1**（在 `android/app/build.gradle`）。当前已到 97（v2.74）。
...
## 已完成的适配
- ✅ 微信输入法打不出中文修复(vc97/v2.74):真机反馈「微信输入法下,记一笔的备注打不了中文」。**根因**:Svelte `bind:value` 在每次 `input` 事件后把值写回 DOM 的 `value`;中文输入法**合成中(拼音选字)**若被写回 value 会打断合成 —— **微信输入法最敏感**,直接一个字都上不了屏(Gboard/系统输入法较宽容,故只有部分输入法暴露)。修:新增 `src/lib/ime.ts` 的 **`imeSafe` action**(compositionstart→置 composing 标志、合成期间**不同步 state 不回写**;compositionend→合成结束再一次性 `cb(node.value)` 同步),用法把 `bind:value={x}` 改成 `value={x} use:imeSafe={(v)=>x=v}`(单向 value + 动作回填,合成期间 x 不变故 Svelte 不写 DOM)。应用到 3 个中文自由文本框:`TxSheet.svelte`(备注)、`Assets.svelte`(资产名称)、`CatSelect.svelte`(自定义分类/来源名,并把 `onkeydown` Enter 加 `!e.isComposing` 守卫,防合成选字回车误触 confirmAdd)。数字框(金额/利率,`inputmode=decimal`)与英文框(AI 模型名)不用管。**注**:emulator 只有 Gboard、复现不了微信输入法的激进打断,但这是 React/Vue 通用的 CJK-IME 标准修法(compositionstart/end 门控);emulator 回归验证:`freegrid://record/expense` 直开记支出 → 备注框输入 abc123 正常显示、光标正常。真机微信输入法待验证。`svelte-check` 0 error。
- ✅ 版面比例锁定 · 竖屏锁定(vc96/v2.73):承 vc95 —— OPPO Find X8 等即便密度归一仍偏大,**根因是不同机型出厂 CSS 视口宽度不同**(小米 14 Pro device-width≈411px、OPPO Find X8≈372px),固定 px 版面在更窄视口上占屏比更大 = "被放大"。vc95 只抹平了用户的「显示大小/字体大小」设置,抹不平机型固有视口差。修(用户选:参考小米 411 + 竖屏锁定):① `index.html` 视口 `width=device-width, initial-scale=1.0` → **`width=411, viewport-fit=cover`**(锁参考宽度=小米基准,各机型等比缩放填满屏宽 → 模块比例/位置全机型一致;**不加 initial-scale**,否则视口更宽机型会被 `max(width,device-width)` 解锁)。② **关键坑**:Capacitor 不设 `useWideViewPort`(默认 **false**),WebView 会**忽略** meta 里的固定 `width=411`、仍按 device-width 布局 → 改了没效果。必须在 `MainActivity.onCreate` 补 `ws.setUseWideViewPort(true)` + `ws.setLoadWithOverviewMode(true)`,WebView 才认固定视口宽并缩放填屏。③ `AndroidManifest.xml` MainActivity 加 `android:screenOrientation="portrait"`(锁定视口宽后横屏会把 411 拉满整宽=巨大,必须锁竖屏)。验证法:`adb shell wm size 1200x2666`(模拟更窄视口机型;**注意用 `wm size` 不是 `wm density`——density 被 vc95 的 `DENSITY_DEVICE_STABLE` 覆盖抹平了,改不出差异**)→ 装 App → 版面比例与 1440 原生**完全一致**(标题不折行、网格/统计卡位置相同)=锁定成功;`wm size reset` 还原。竖屏:`settings put system user_rotation 1` 强制横屏后 App 仍竖屏(`dumpsys window` 见 `mCurrentAppOrientation=SCREEN_ORIENTATION_PORTRAIT`)。**取舍**:平板/折叠展开会把手机版面等比放大填充(可接受);已锁竖屏故无横屏问题。
